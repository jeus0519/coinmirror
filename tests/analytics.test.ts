...
test('Google tag bootstrap은 기본 page_view를 끄고 쿼리 없는 page_path를 수동 전송한다', async () => {
  const source = await readFile('src/lib/analytics.ts', 'utf8');
  assert.match(source, /send_page_view:\s*false/);
  assert.match(source, /sanitizePagePath/);
  assert.doesNotMatch(source, /window\.location\.search/);
});

test('Vercel 설정은 기본 보안 헤더를 포함한다', async () => {
...
test('업로드 화면은 파일 크기와 PDF 페이지 제한을 소스에서 강제한다', async () => {
  const upload = await readFile('src/components/steps/step-3-data-import.tsx', 'utf8');
  const pdf = await readFile('src/lib/pdf/extract-pdf-text.ts', 'utf8');
  assert.match(upload, /MAX_UPLOAD_FILE_BYTES/);
  assert.match(upload, /asset\.size/);
  assert.match(pdf, /MAX_PDF_PAGE_COUNT/);
});


test('Google tag 수동 page_view는 page_location에서도 query token을 제거한다', () => {
  const originalWindow = globalThis.window;
  Object.defineProperty(globalThis, 'window', {
    configurable: true,
    value: { location: { origin: 'https://coinmirror.app', pathname: '/', href: 'https://coinmirror.app/?source=reanalysis-email&r=secret-token' }, document: { referrer: 'https://example.com/?email=user@example.com' } },
  });
  const calls: unknown[][] = [];
  const doc = {
    createElement: () => ({}),
  ...
  installGoogleTag({ enabled: true, measurementId: 'G-TEST' }, doc, (...args) => calls.push(args));
