...
| Vercel 배포·WAF | 🟢 Preview·WAF·Git 연결 완료 | `data-grida/coinmirror` Preview가 `READY`이고 Node.js 24.x, static route 7개, API route 3개를 확인했다. POST AI route에 IP·Fixed Window·60초 10회 edge WAF rule 1개가 live이며 병렬 합성 요청에서 10건 허용 후 4건 429를 확인했다. 원격 환경변수는 0개이고 `jeus0519/coinmirror`의 Production Branch를 별도 `production`으로 고정해 `main` push는 Preview만 생성한다. 활성 Production alias는 0개다. |
