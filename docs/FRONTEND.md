...
- 탭/스택 구조 변경은 `src/app/_layout.tsx`와 `src/components/app-tabs.tsx`를 함께 확인할 것.
...
- 화면 전용 조합 컴포넌트는 `src/components/` 바로 아래, 재사용 컴포넌트는 `src/components/ui/`.
...
- 화면을 넘나드는 상태는 Zustand 스토어(`src/stores/*.ts`)로. 패턴 예시: `src/stores/use-journal-store.ts`.
...
- 감정/편향 라벨링(공포, 탐욕, FOMO 등)이 판단을 돕는 방식으로 노출되는가
