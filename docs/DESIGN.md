...
분석 화면은 “근거 → 타입 요약 → 선택형 AI 회고 → 자기인식 비교 → 상세 근거” 순서로 읽히게 한다.

```text
기록 분석 → 거래 개요 → 투자거울 타입 → AI 행동코칭 → 내 예상 vs 기록 → 행동 점수 → 이번 분석에서 눈에 띄는 패턴 → 다음 달 비교 준비 → 월간 투자습관 리포트 → 구독 전용 지표 → 차트/종목 비중 → 피드백
```

- `거래 개요`는 승률, 보유기간, 거래 횟수 등 사용자가 바로 이해할 수 있는 핵심 지표를 먼저 보여준다.
- `투자거울 타입`은 거래 개요 직후 배치해 최근 거래 기록의 전체 성향을 기억하기 쉬운 이름으로 먼저 요약한다.
- `AI 행동코칭`은 타입 다음에 선택형 버튼으로 제공한다. 문구는 줄여볼 행동, 유지할 행동, 다음 달 확인 질문으로 제한한다.
