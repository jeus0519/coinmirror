# v1.0 코인미러 대고객 런칭 준비 실행계획

> **For Hermes:** Use subagent-driven-development skill to implement this plan task-by-task.

**Goal:** 데모/지원서 확장을 중단하고, 코인미러를 실제 외부 고객에게 안전하게 열 수 있는 **웹 MVP** 런칭 상태로 전환한다.

**Architecture:** 현재 구현된 업비트 PDF/CSV 분석, 웹 기반 local-first 처리, 중복 업로드 처리, 월간 리포트 미리보기를 유지하되, 런칭 전 게이트를 보안·규제·분석 신뢰성·첫 사용자 UX·웹 배포·피드백 루프로 재정렬한다. 서버/로그인/결제는 바로 구현하지 않고, 외부 검증 가능한 웹 MVP와 수동 결제/대기자/인터뷰 루프로 먼저 수요를 검증한다. 여기서 local-first는 고객 설치형 앱이 아니라 **브라우저 안에서 원본 파일을 우선 처리하고 서버 장기 저장을 피하는 구조**를 뜻한다.
...
6. 결제 전이라도 대기자/인터뷰/수동 구독 의사를 수집할 수 있는가?
...
**목표:** 공개 결제 서비스가 아니라, 제한된 외부 사용자에게 안전하게 열 수 있는 “Closed Beta 웹 MVP”를 만든다.
...
| P1 | 대기자/인터뷰 전환 | 미구현 | 랜딩 CTA와 수동 신청 폼/메일 링크 추가 |
| P1 | 배포 | 미정 | 웹 배포 후보 선택 및 preview/prod 분리 |
| P2 | 결제/구독 | 화면 preview만 있음 | Closed Beta 후 수동 결제/PG 결정 |
...
### Week 2 — Closed Beta 운영 준비

**목표:** 제한된 고객에게 열고 피드백을 수집한다.

1. 배포 환경 오픈
2. 첫 사용자 체크리스트 작성
3. 피드백 폼/인터뷰 스크립트 연결
4. 5~10명에게 수동 배포
5. 업로드 성공률/이해도/구독 의향 기록
6. 치명 이슈 수정
7. 유료화 진입 여부 판단
...
### Task 6: Closed Beta CTA 추가

**Objective:** 결제 전에도 실제 고객 수요를 모을 수 있게 대기자/인터뷰 신청 CTA를 만든다.
...
**Recommended Default:** Google Form/Tally URL은 사용자가 정하면 연결. 그 전까지는 `mailto:` CTA로 구현.
...
데모는 충분하다. 다음 스프린트의 이름은 “데모 고도화”가 아니라 **Closed Beta Launch Readiness**가 되어야 한다.
...
이 3개가 끝나야 실제 사용자에게 링크를 보낼 수 있다.
